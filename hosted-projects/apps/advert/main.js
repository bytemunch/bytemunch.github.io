...
class Person {
    ...
    get motive() {
        return this.motiveBalancer.currentMotive;
    }
    get cx() {
    ...
    createLikes() {
        for (let p of allProducts) {
            ...
            if (n > 2 / 3)
                this.likes.push(p);
            if (n < 1 / 3)
                this.dislikes.push(p);
        }
        if (this.likes.length == 0) {
            this.likes.push(randomProduct());
            if (this.dislikes.indexOf(this.likes[0]) !== -1)
                this.dislikes.splice(this.dislikes.indexOf(this.likes[0]), 1);
        }
    }
    createColor() {
    ...
    walkTo(target, shop) {
        if (this.motive == 'home' && this.atHome)
            return;
        this.atHome = false;
    ...
    buy(product) {
        ...
        this.motiveBalancer.loseMotivation(product.type, 1);
    }
    earn() {
        this.wealth += 25;
    ...
    update() {
        if (this.wealth < 0)
            this.wealth = 0;
        this.updateMotives();
        if (this.atHome && !this.earnedThisRest)
            this.earn();
...
class ProductButton extends Button {
    constructor(x, y, w, h, type) {
        ...
        this.unlocked = false;
        this.clickable = true;
        this.slot1 = new ProductSlot(this.x + 4, this.y + 4, 48, 48, this.type);
        this.slot2 = new ProductSlot(this.x + 4 + 48 + 4, this.y + 4, 48, 48, this.type);
...
class ProductSlot extends Button {
    constructor(x, y, w, h, productType) {
        super(x, y, w, h, 'empty');
        this.color = '#69696930';
        this.type = 'empty';
        this.count = 0;
...
class AdButton extends Button {
    ...
    use() {
        ...
        this.cooldown = 0;
        if (!bank.decrement(AdManifest[this.type].cost))
            return false;
    }
    unlock() {
        // console.log('unlock');
        if (this.unlocked)
            return;
        if (!bank.decrement(AdManifest[this.type].unlock))
            return false;
        this.unlocked = true;
...
class MotiveBalancer {
    ...
    addMotivation(type, amt) {
        this[type] += amt;
        if (this[type] < 0)
            this[type] = 0;
    }
    loseMotivation(type, amt) {
...
class CashPickup {
    ...
    update() {
        if (framecount - this.startFrame > this.lifespan)
            this.die();
        this.y--;
...
function clickHandler(e) {
    ...
    for (let a of allActors()) {
        if (a.clickable && x > a.x && x < a.x + a.width && y > a.y && y < a.y + a.height)
            target = a;
    }
    if (target)
        target.click();
...
function touchHandler(e) {
    ...
    touched = undefined;
    for (let a of allActors()) {
        if (a.draggable && x > a.x && x < a.x + a.width && y > a.y && y < a.y + a.height)
            touched = a;
    }
    if (touched) {
        // touched.click();
        ...
        const dropHandler = (e) => {
            ...
            function dropThing() {
                let target;
                for (let a of allActors()) {
                    if (a.droppable && tX > a.x && tX < a.x + a.width && tY > a.y && tY < a.y + a.height)
                        target = a;
                }
                if (target && target.drop(touched))
                    touched.use();
            }
